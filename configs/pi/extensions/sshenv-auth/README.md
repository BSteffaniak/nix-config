...
- Sessions persist in shared `~/.pi/agent/sessions/` by default via `~/.pi/<profile>/agent/sessions` symlinks, so pi keeps its normal working-directory organization while profiles share the same project history.
