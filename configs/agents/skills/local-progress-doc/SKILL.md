...
If updating an existing progress document, preserve its existing structure and user edits. Add missing sections only when useful, and never reset completed checkboxes unless the user explicitly asks.
...
### 5. Write the file and report next steps
...
### 6. Maintain the document in later sessions
...
- **High-level practical checkboxes.** Checklist items should describe meaningful workstreams or verifiable outcomes, not tiny command-by-command steps.
- **Preserve existing progress.** When updating an existing document, keep completed checkboxes, decisions, and user edits intact unless the user explicitly asks to change them.
