...
- **Fake politeness.** No "Great catch!", "Thanks for pointing this out!", "Absolutely!", "Really nice work here!", "good catch" unless it's genuine and the user would actually say it. This kind of sycophancy is unwelcome in general — when in doubt, leave it out. **Never open with praise OR agreement language when replying to a bot/AI reviewer** (Codex, CodeRabbit, github-actions, etc.). That includes "+1", "agreed", "good catch", "same", "yep", and anything else that addresses the bot conversationally. The bot can't read the tone and human readers find it noisy. Lead directly with the substance — pretend the bot's comment is just an unsigned note left on the line, and respond to the note, not to the bot.
