...
description: Generate a commit message from staged changes only and write it so it's pre-filled when you run `git commit` or press `c` in lazygit.
allowed-tools: Bash(git:*)
---

## Purpose

Generate a commit message from currently staged changes ONLY, and write it to the repo so it's automatically pre-filled in both:
