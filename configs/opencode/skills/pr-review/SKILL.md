---
name: pr-review
description: Fetch and address PR review comments. Categorizes comments as actionable or informational, skips resolved threads, and makes code changes.
allowed-tools: Bash(gh:*), Bash(git:*), Bash(jq:*)
---

## Purpose

Fetch all review feedback on a pull request, categorize each comment as actionable or non-actionable, skip resolved/outdated threads, present a structured plan, and then make code changes for the comments the user chooses to address.
...
1. `src/baz.ts:8` — **@reviewer**: "<question excerpt>"
...
- **Never act without user confirmation.** Always present the plan first (Step 4-5) and wait for explicit go-ahead before making any code changes.
