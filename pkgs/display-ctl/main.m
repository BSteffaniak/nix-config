...
typedef int32_t (*DSEnableAmbientLightCompensation)(CGDirectDisplayID display,
                                                    Boolean enable);
...
static BOOL setTrueTone(BOOL enabled) {
  void *handle = dlopen("/System/Library/PrivateFrameworks/"
                        "CoreBrightness.framework/CoreBrightness",
                        RTLD_LAZY);
  if (!handle) {
  ...
  BoolMethod supportedFunc =
      (BoolMethod)[client methodForSelector:selSupported];
  ...
  BoolMethod availableFunc =
      (BoolMethod)[client methodForSelector:selAvailable];
  ...
  SetEnabledMethod setEnabledFunc =
      (SetEnabledMethod)[client methodForSelector:selSetEnabled];
...
static BOOL setAutoBrightness(BOOL enabled) {
  ...
  DSHasAmbientLightCompensation hasALC =
      dlsym(handle, "DisplayServicesHasAmbientLightCompensation");
  DSEnableAmbientLightCompensation enableALC =
      dlsym(handle, "DisplayServicesEnableAmbientLightCompensation");

  if (!hasALC || !enableALC) {
    fprintf(stderr,
            "error: DisplayServices ambient light functions not found\n");
    dlclose(handle);
    return NO;
  }
  ...
  if (!hasALC(displayID)) {
    fprintf(
        stderr,
        "error: ambient light compensation not available on this display\n");
    dlclose(handle);
    return NO;
  }
    ...
    for (int i = 1; i < argc; i++) {
      if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
        ...
        if (strcmp(argv[i], "on") == 0) {
          ...
          fprintf(stderr,
                  "error: --auto-brightness requires on|off, got '%s'\n",
                  argv[i]);
          return 1;
        }
        didSomething = YES;
