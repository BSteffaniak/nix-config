    ...
    let
      ...
      rustToolchains =
        if hasStable && hasNightly then
          # Both stable and nightly: use wrappers only
          # Wrappers provide rustc, cargo, rust-analyzer, and all tools for both versions
          [
            (pkgs.mkCargoWrapper {
            ...
            # Note: Don't include stableToolchain here - causes collision with wrappers
            # The wrappers already provide access to stable toolchain
          ]
